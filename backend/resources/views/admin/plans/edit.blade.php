...
<div class="card" style="max-width:600px;">
    ...
    <form action="{{ route('admin.plans.update', $plan->id) }}" method="POST">
        ...
        <div style="margin-bottom:1.25rem;">
            ...
            <input type="text" name="name" value="{{ old('name', $plan->name) }}" required style="width:100%;padding:0.75rem 1rem;background:rgba(0,0,0,0.3);border:1px solid var(--bg-card-border);border-radius:8px;color:#fff;outline:none;">
        </div>

        <div style="margin-bottom:1.25rem;">
            <label style="display:block;font-size:0.85rem;font-weight:600;color:var(--text-muted);margin-bottom:0.4rem;">Deskripsi</label>
            <textarea name="description" rows="3" style="width:100%;padding:0.75rem 1rem;background:rgba(0,0,0,0.3);border:1px solid var(--bg-card-border);border-radius:8px;color:#fff;outline:none;">{{ old('description', $plan->description) }}</textarea>
        </div>

        <div style="margin-bottom:1.25rem;">
            <label style="display:block;font-size:0.85rem;font-weight:600;color:var(--text-muted);margin-bottom:0.4rem;">Daily Limit</label>
            <input type="number" name="daily_limit" value="{{ old('daily_limit', $plan->daily_limit) }}" min="1" style="width:100%;padding:0.75rem 1rem;background:rgba(0,0,0,0.3);border:1px solid var(--bg-card-border);border-radius:8px;color:#fff;outline:none;">
        </div>

        <div style="margin-bottom:1.25rem;display:flex;align-items:center;gap:10px;">
            <input type="checkbox" name="unlimited" value="1" id="unlimitedPlan" {{ old('unlimited', $plan->unlimited) ? 'checked' : '' }}>
        ...
        <div class="price-currency-grid">
            ...
            <div>
                ...
                <input type="text" name="currency" value="{{ old('currency', $plan->currency) }}" required style="width:100%;padding:0.75rem 1rem;background:rgba(0,0,0,0.3);border:1px solid var(--bg-card-border);border-radius:8px;color:#fff;outline:none;">
            </div>
        </div>

        <div style="margin-bottom:1.75rem;display:flex;align-items:center;gap:10px;">
            <input type="checkbox" name="is_active" value="1" id="activePlan" {{ old('is_active', $plan->is_active) ? 'checked' : '' }}>
